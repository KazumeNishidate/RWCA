...
/* re-scale all of the atomic positions */
    ...
    y_scale = 450.0/ (double)(ctl.mat_size+4);
  ...
  for (i = 2; i < ctl.mat_size+2; i++){
	...
	xypts[ii].y=(int)( (double)j*y_scale)+30;
/*
...
/* ====== Real Time Step ======= */
  XSetForeground (d,gc,color[black]);  /* Refresh the display */
  XFillRectangle(d,w0,gc,0,0,480,500);

  XSetForeground (d,gc,color[white]);  /* timestep indicator */
  sprintf(stringbuffer,"TimeStep = %d ",sys.time_step);
  XDrawString(d,w0,gc,15,20,stringbuffer,strlen(stringbuffer));

  XSetForeground (d,gc,color[green]);  /* walkers            */
