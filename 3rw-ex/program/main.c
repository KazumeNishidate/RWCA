...
/* RWCA-3d uder cyclic boundary condition */
...
*/
    ...
    /*----------- RWCA moving step -----------*/
    for(sys.time_step=1;sys.time_step<=ctl.max_time_step;sys.time_step++)
      {
/*-------------- X interface -------------------*/
#ifdef XD
	xd();
...
/* check */
	...
	printf("run[%d] time = %d  ",sys.average_step,sys.time_step); 
